...
function waitForReadyLine(child: ChildProcess): Promise<string> {
  return new Promise((resolveReady, reject) => {
    ...
    const onData = (chunk: Buffer): void => {
      out += chunk.toString()
      const match = /Hyperlake SuperHarness: (http:\/\/[^\s]+)/.exec(out)
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  it('empty-state first send completes a real model round', async () => {
    ...
    await page.waitForFunction(
      () => document.title !== 'Hyperlake SuperHarness' && document.title.endsWith(' — Hyperlake SuperHarness'),
      undefined,
      { timeout: 15_000 },
    )
    ...
    await page.waitForFunction(
      expected => document.title === `${expected} — Hyperlake SuperHarness`,
      durableTitle,
      { timeout: 15_000 },
    )
