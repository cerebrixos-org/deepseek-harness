...
const PROCESS_READY_TIMEOUT_MS = 120_000
const HMR_UPDATE_TIMEOUT_MS = 60_000
...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  ...
  try {
    ...
    const baseUrl = await waitForOutput(host, /Hyperlake SuperHarness: (http:\/\/[^\s]+)/, 'built Hyperlake SuperHarness web')
    ...
    await page.getByText(newText, { exact: true }).waitFor({ timeout: HMR_UPDATE_TIMEOUT_MS })
