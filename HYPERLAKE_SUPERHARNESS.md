...
Hyperlake SuperHarness is a downstream DeepSeek Harness composition for governed infrastructure and data work. It preserves the Harness runtime and plugin model while mounting existing Hyperlake CLI/MCP capabilities and installable industry-pack content.
...
3. Adapter plugins provide capability ids and mount existing tool surfaces. The Hyperlake adapter starts `hyperlake mcp serve --stdio` unchanged; the optional Databricks adapter starts a customer-selected local MCP command.
...
Pack validation does not grant access. It proves composition readiness; Hyperlake, Databricks, or another target still enforces identity, policy, and resource authorization.

## Profiles and Packs

The shipped `hyperlake` profile composes the base web Harness, the SuperHarness registry, and the Hyperlake adapter. Set `SUPERHARNESS_HYPERLAKE_DISABLED=1` to inspect or boot the profile without starting the CLI MCP process.
