...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  /** Settings namespaces available through the remote configuration boundary. */
  function exposedNamespaces(): Set<string> {
    const exposed = new Set(ctx.llm.listConfigurableProviders().map(entry => entry.settingsNs))
    for (const ns of WEB_SETTINGS_NAMESPACES) exposed.add(ns)
    for (const ns of PRODUCT_SETTINGS_NAMESPACES) exposed.add(ns)
    return exposed
  }

  /** Refuse settings that were not explicitly selected for remote configuration. */
      ...
      describe(request) {
        ...
        return Promise.resolve(ok(request, {
          ...
          namespaces: settings.describe({ redactSecrets: true })
            .filter(descriptor => exposedNamespaces().has(String(descriptor.ns)))
            .map(namespaceView),
        }))
      },
      async openDocument(request, signal) {
