...
Capabilities declare explicit user-facing outcomes. An outcome may define non-secret inputs, required resource slots, a tool or deterministic workflow entry point, approval mode, and evaluation gates. Core Harness tools are universal and cannot be redundantly attached. A deployment may attach optional installed-plugin tools either globally, where they are cross-cutting, or to one or more outcomes in a capability. It may also discover provider-owned resources, store their opaque ids, and attach plugin-exported knowledge, workflows, or evaluations with immutable source provenance. Selecting an outcome snapshots this contract into the session and exposes only global tools plus tools mapped to that outcome. Agent-scoped restrictions hide tools assigned to other capabilities or outcomes, and an execution guard preserves the same rule after session resume.
