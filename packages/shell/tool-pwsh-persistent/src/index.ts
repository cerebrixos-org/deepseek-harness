  ...
  const get = (owner: Agent, signal: AbortSignal): Promise<TerminalSessionId> => {
    ...
    const creation = (async () => {
      try {
        ...
        // Terminal backends return from spawn only after their prompt is ready.
        // Replacing that prompt here would invalidate the backend's readiness
        // detector and let the first command race a second initialization.
